...
import { Link2, ExternalLink, Globe, FileText, Link as LinkIcon, AlertTriangle } from "lucide-react"
...
export default function LinksTab({ links }: { links: LinkElement[] }) {
    const [filter, setFilter] = useState<'all' | 'internal' | 'subdomain' | 'external' | 'resource' | 'broken'>('all');
        ...
        return (
            <Card>
                <CardContent className="py-12 text-center">
                    ...
                    <p className="text-sm text-muted-foreground mt-1">Check settings to ensure links are being crawled</p>
                </CardContent>
            </Card>
        )
    }
                                ...
                                <TableRow key={idx}>
                                    ...
                                    <TableCell className="text-center">
                                        <Badge variant="outline" className={cn(
                                            "text-xs",
                                            link.link_type === 'External' ? "bg-muted" :
                                                link.link_type === 'Subdomain' ? "bg-blue-100 text-blue-800 border-blue-200" :
                                                    "bg-primary/15 text-primary border-primary/20"
                                        )}>
                                            {link.link_type === 'External' ? "Ext" : link.link_type === 'Subdomain' ? "Sub" : "Int"}
                                        </Badge>
                                    </TableCell>
                                    <TableCell className="text-center">
                                        {link.status_code ? (
                                            <Badge variant="outline" className={cn(
                                                "text-xs font-mono",
                                                link.status_code >= 200 && link.status_code < 300 ? "bg-success/15 text-success border-success/20" :
                                                    link.status_code >= 400 ? "bg-destructive/15 text-destructive border-destructive/20" :
