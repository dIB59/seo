    ...
    return (
        <div className="min-h-screen bg-background text-foreground relative overflow-hidden">
            ...
            <div className="relative z-10 p-4 space-y-6 max-w-[1600px] mx-auto">
                ...
                {/* Data View Tabs */}
                <Tabs defaultValue="issues" className="animate-in fade-in slide-in-from-bottom-4 duration-500 delay-200">
                    <div className="flex items-center justify-between mb-4">
                        <TabsList className="bg-muted/30 border border-border/40 p-1 h-auto backdrop-blur-sm shadow-sm">
                            <TabsTrigger value="issues" className="gap-2 px-4 py-2 data-[state=active]:bg-background data-[state=active]:shadow-sm transition-all duration-300 hover:bg-muted/20 hover:text-foreground">
                                <AlertTriangle className="h-4 w-4" />
                                Issues
                            </TabsTrigger>
                            <TabsTrigger value="pages" className="gap-2 px-4 py-2 data-[state=active]:bg-background data-[state=active]:shadow-sm transition-all duration-300 hover:bg-muted/20 hover:text-foreground">
                                <FileText className="h-4 w-4" />
                                Pages
                            </TabsTrigger>
                            <TabsTrigger value="graph" className="gap-2 px-4 py-2 data-[state=active]:bg-background data-[state=active]:shadow-sm transition-all duration-300 hover:bg-muted/20 hover:text-foreground">
                                <Network className="h-4 w-4" />
                                Site Graph
                            </TabsTrigger>
                            <TabsTrigger value="overview" className="gap-2 px-4 py-2 data-[state=active]:bg-background data-[state=active]:shadow-sm transition-all duration-300 hover:bg-muted/20 hover:text-foreground">
                    ...
                    <div className="mt-4 min-h-[500px]">
                        ...
                        <TabsContent value="graph" className="mt-0 h-[600px] rounded-lg overflow-hidden border border-border/40 bg-background/50">
                            <GraphView
                                data={data}
                                onNodeClick={(page) => {
                                    console.log(page);
