        ...
        <div className="overflow-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <StatItemError
              icon={FileCode}
              label="Content Size"
              value={`${(page.content_size / 1024).toFixed(1)}KB`}
            />
            <StatItemError
              icon={Link2}
              label="Links"
              value={`${page.internal_links}/${page.external_links}`}
            />
          </div>
        </div>
      </DialogContent>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
              <MetricBadge
                label="H1 Tags"
                value={page.headings.filter((h) => h.tag === "h1").length}
              />
              <MetricBadge
                label="H2 Tags"
                value={page.headings.filter((h) => h.tag === "h2").length}
              />
              <MetricBadge
                label="H3 Tags"
                value={page.headings.filter((h) => h.tag === "h3").length}
              />
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {page.images_without_alt > 0 && (
              <Badge
                variant="outline"
                className="text-xs bg-destructive/15 text-destructive border-destructive/20"
              >
