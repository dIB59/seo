...
export function JobItem({ job, onViewResult, onCancel }: JobItemProps) {
  const { count: pagesDiscovered, total: discoveryTotal } = useDiscoveryProgress(
    job.job_id,
    job.job_status,
  );
  const { current: pagesAnalyzed, total: analysisTotal } = useAnalysisProgress(
    job.job_id,
    job.job_status,
  );
  ...
  return (
    <div className="group relative flex items-center gap-4 p-3 bg-card/40 hover:bg-card/90 border-b border-border/50 first:border-t hover:border-transparent hover:shadow-sm transition-all duration-200">
      {/* Status Column */}
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-center gap-1.5 py-1">
        ...
        <div className="flex items-center gap-2">
          <span className="font-medium text-sm truncate text-foreground/90 tracking-tight">
            {job.url}
          </span>
        </div>
        ...
        {(isAnalyzing || isDiscovering) && (
          <div className="max-w-[200px]">
            ...
            {isDiscovering && discoveryTotal !== null && (
              <JobProgressBar
                current={pagesDiscovered ?? 0}
                total={discoveryTotal}
                label={
                  !discoveryTotal
                    ? `Discovered ${pagesDiscovered ?? 0} / ${discoveryTotal}`
                    : undefined
                }
              />
            )}
      ...
      <div className="hidden lg:flex items-center gap-6 mr-6">
        ...
        <div className="flex flex-col items-end gap-1.5 w-16">
          ...
          <span className="text-xs font-mono text-muted-foreground/80 leading-none">
            {job.is_deep_audit ? "DEEP" : "STANDARD"}
          </span>
        </div>
