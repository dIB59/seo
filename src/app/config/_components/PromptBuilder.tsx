...
function SortableBlock({ block, onRemove, onUpdate }: { block: PromptBlock, onRemove: (id: string) => void, onUpdate: (id: string, val: string) => void }) {
    ...
    return (
        <div ref={setNodeRef} style={style} className="flex gap-2 bg-card p-3 rounded-lg border border-border/50 hover:border-border/80 transition-colors group mb-3 relative shadow-sm">
            {/* Controls */}
            ...
            <div className="flex-1 space-y-2">
                <div className="flex justify-between items-center">
                    <span className={`text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-full border ${block.type === 'variable' ? 'text-blue-500 bg-blue-500/10 border-blue-500/20' : 'text-zinc-500 bg-zinc-500/10 border-zinc-500/20'}`}>
                        {block.type === 'variable' ? 'Data Variable' : 'Instruction Text'}
                    </span>
                    <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-destructive opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => onRemove(block.id)}>
                ...
                <Textarea
                    ...
                    placeholder={block.type === 'variable' ? "Variable template (e.g. Score: {score})" : "Enter instructions for the AI..."}
                />
            </div>
        </div>
    );
