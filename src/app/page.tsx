		...
		<div className="bg-blue-50 dark:bg-blue-900/20 rounded-2xl p-6 border border-blue-200 dark:border-blue-800">
				...
				<div className="flex items-center gap-2">
					<div className="flex items-center text-blue-600 dark:text-blue-400">
						...
						<span className="text-sm font-medium">
							{currentAnalysis!.analysis_status === 'paused' ? 'Paused' : 'In Progress'}
						</span>
					</div>
					{currentAnalysis!.analysis_status === 'analyzing' && (
						<button
							onClick={() => pauseAnalysis(currentAnalysis!.job_id)}
							className="px-3 py-1 bg-yellow-500 hover:bg-yellow-600 text-white text-sm rounded-md transition-colors"
						>
							Pause
						</button>
					)}
					{currentAnalysis!.analysis_status === 'paused' && (
						<button
							onClick={() => resumeAnalysis(currentAnalysis!.job_id)}
			...
			<div className="space-y-3">
				<div className="flex justify-between text-sm text-blue-700 dark:text-blue-300">
					<span>Progress</span>
					<span>{Math.round(currentAnalysis?.progress ?? 0)}%</span>
				</div>
