...
export class Result<T, E = string> {
  ...
  unwrapOr(fb: T): T {
    return this.isOk() ? (this._value as T) : fb;
  }
  ...
  async matchAsync<U>(
    ...
    return this.isOk() ? await onOk(this._value as T) : await onErr(this._error as E);
  }
}
...
async function fromPromise<T>(p: Promise<T>): Promise<Result<T, string>> {
  return p
    .then((v) => {
      logger.info(v);
      return Result.Ok(v);
