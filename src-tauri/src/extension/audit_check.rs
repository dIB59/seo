...
mod tests {
    ...
    fn test_image_alt_check() {
        let html = r#"<html><body><img src="a.jpg" alt="A"><img src="b.jpg"></body></html>"#;
        let context = make_context(html, "https://example.com");
        ...
        assert!(!result.passed);
        assert!(result.description.unwrap().contains("1 images missing"));
    }

    #[test]
    fn test_robots_meta_check_noindex() {
        let html = r#"<html><head><meta name="robots" content="noindex, nofollow"></head></html>"#;
        let context = make_context(html, "https://example.com");

        let check = RobotsMetaCheck::new();
        let result = check.check(&context);

        assert!(!result.passed);
        assert!(result.description.unwrap().contains("noindex"));
    }
}
