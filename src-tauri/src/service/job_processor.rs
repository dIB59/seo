...
/// Orchestrates SEO analysis jobs using the schema.
///
    ...
    async fn analyze_page(
        ...
        let (page, extracted_issues, new_urls, edges) = {
            ...
            if title.is_none() || title.as_ref().map(|t| t.is_empty()).unwrap_or(true) {
                issues.push(("Missing Title".to_string(), "Page has no title tag".to_string(), "Add a descriptive title tag".to_string(), IssueSeverity::Critical));
            }
            
            if meta_description.is_none() || meta_description.as_ref().map(|d| d.is_empty()).unwrap_or(true) {
                issues.push(("Missing Meta Description".to_string(), "Page has no meta description".to_string(), "Add a meta description".to_string(), IssueSeverity::Warning));
            }
            
            if audit_result.status_code >= 400 {
                issues.push(("HTTP Error".to_string(), format!("Page returned status code {}", audit_result.status_code), "Fix the HTTP error".to_string(), IssueSeverity::Critical));
            }
...
impl PageEdge {
    ...
    fn is_internal(&self, base_url: &str) -> bool {
        if let (Ok(edge_url), Ok(base)) = (Url::parse(&self.to_url), Url::parse(base_url)) {
            edge_url.host_str() == base.host_str()
        } else {
            false
        }
    }
