...
impl LighthouseService {
    ...
    pub async fn start_persistent(&self) -> Result<()> {
        ...
        if process.is_some() {
            tracing::debug!("[LIGHTHOUSE-SIDECAR] Persistent process already running");
            return Ok(());
        }
        ...
        tracing::info!("[LIGHTHOUSE-SIDECAR] Starting persistent sidecar process...");
        ...
        tracing::info!("[LIGHTHOUSE-SIDECAR] Persistent process started and ready");
    ...
    async fn analyze_persistent(&self, url: &str) -> Result<PageFetchResult> {
        ...
        tracing::debug!(
            "[LIGHTHOUSE-SIDECAR] [PERSISTENT] Sending request for: {}",
            url
        );
        ...
        tracing::debug!(
            "[LIGHTHOUSE-SIDECAR] [PERSISTENT] Response received in {:.2}ms",
            process_time_ms
        );
        ...
        let actual_load_time_ms = scores
            .performance_metrics
            .as_ref()
...
info!(
    ...
    pub async fn analyze(&self, url: &str) -> Result<PageFetchResult> {
        tracing::info!("[LIGHTHOUSE-SIDECAR] Starting analysis for: {}", url);
    ...
    async fn analyze_oneshot(&self, url: &str) -> Result<PageFetchResult> {
        tracing::debug!("[LIGHTHOUSE-SIDECAR] Using one-shot mode for: {}", url);

        if !self.is_available() {
            tracing::error!(
                "[LIGHTHOUSE-SIDECAR] Binary not found at: {:?}",
                self.sidecar_path
            );
            anyhow::bail!(
                "Lighthouse sidecar binary not found at: {:?}. This is a packaging error.",
        ...
        tracing::debug!("[LIGHTHOUSE-SIDECAR] Binary verified, spawning process...");

        let start_time = std::time::Instant::now();

        // Spawn the sidecar binary directly
        tracing::debug!(
            "[LIGHTHOUSE-SIDECAR] Executing command: {:?} {}",
            self.sidecar_path,
            url
        );
        ...
        let process_time_ms = start_time.elapsed().as_secs_f64() * 1000.0;
        tracing::info!(
            "[LIGHTHOUSE-SIDECAR] Process completed in {:.2}ms",
            process_time_ms
        );
        ...
        tracing::trace!("[LIGHTHOUSE-SIDECAR] stdout length: {} bytes", stdout.len());
        if !stderr.is_empty() {
            tracing::debug!("[LIGHTHOUSE-SIDECAR] stderr: {}", stderr.trim());
        }

        if !output.status.success() {
            tracing::error!(
                "[LIGHTHOUSE-SIDECAR] Process failed with status: {:?}",
                output.status
            );
            tracing::error!("[LIGHTHOUSE-SIDECAR] stderr: {}", stderr);
            anyhow::bail!(
        ...
        tracing::debug!("[LIGHTHOUSE-SIDECAR] Parsing JSON response...");
        let response: SidecarResponse =
            serde_json::from_str(&stdout).context("Failed to parse lighthouse-runner output")?;
        ...
        if !response.success {
            let error_msg = response
                ...
                .unwrap_or_else(|| "Unknown error".to_string());
            tracing::error!(
                "[LIGHTHOUSE-SIDECAR] Analysis reported failure: {}",
                error_msg
            );
            anyhow::bail!("Lighthouse analysis failed: {}", error_msg);
        }
        ...
        let html = if let Some(ref h) = response.html {
            if !h.is_empty() {
                tracing::debug!(
                    "[LIGHTHOUSE-SIDECAR] Using rendered HTML from Lighthouse ({} bytes)",
                    h.len()
                );
                h.clone()
            } else {
                tracing::warn!(
                    "[LIGHTHOUSE-SIDECAR] Lighthouse returned empty HTML, fetching separately"
                );
                self.fetch_html(url).await.unwrap_or_default()
            }
        } else {
            tracing::warn!(
                "[LIGHTHOUSE-SIDECAR] Lighthouse didn't return HTML, fetching separately"
            );
            self.fetch_html(url).await.unwrap_or_default()
        };
...
info!(
    ...
    pub async fn analyze_urls(&self, urls: &[String]) -> Vec<Result<PageFetchResult>> {
        ...
        tracing::info!(
            "[LIGHTHOUSE-SIDECAR] Analyzing {} URLs with persistent mode",
            urls.len()
        );
        ...
        if let Err(e) = self.start_persistent().await {
            tracing::
warn!("[LIGHTHOUSE-SIDECAR] Failed to start persistent mode: {}, falling back to one-shot", e);
        }
    ...
    pub async fn shutdown(&self) -> Result<()> {
        ...
        if let Some(mut p) = process.take() {
            ...
            let pid = p.child.id();
            tracing::info!(
                "[LIGHTHOUSE-SIDECAR] Shutting down persistent process (pid: {:?})",
                pid
            );
            ...
            match tokio::time::timeout(wait_duration, p.child.wait()).await {
                Ok(Ok(status)) => {
                    tracing::info!(
                        "[LIGHTHOUSE-SIDECAR] Persistent process exited gracefully: {:?}",
                        status
                    );
                }
                Ok(Err(e)) => {
                    tracing::
warn!("[LIGHTHOUSE-SIDECAR] Error while waiting for child to exit: {}. Attempting to kill.", e);
                    let _ = p.child.kill().await;
                    // Best-effort wait after kill
                    let _ =
                        tokio::time::timeout(tokio::time::Duration::from_secs(1), p.child.wait())
                            .await;
                }
                Err(_) => {
                    tracing::
warn!("[LIGHTHOUSE-SIDECAR] Timeout waiting for persistent process to exit, killing...");
                    let _ = p.child.kill().await;
                    let _ =
                        tokio::time::timeout(tokio::time::Duration::from_secs(1), p.child.wait())
                            .await;
            ...
            tracing::info!("[LIGHTHOUSE-SIDECAR] Persistent process shut down");
        } else {
            tracing::debug!("[LIGHTHOUSE-SIDECAR] No persistent process to shut down");
        }
...
mod tests {
    ...
    async fn test_shutdown_timeout_kill() {
        ...
        let res =
            tokio::time::timeout(tokio::time::Duration::from_secs(5), service.shutdown()).await;
        assert!(res.is_ok());
