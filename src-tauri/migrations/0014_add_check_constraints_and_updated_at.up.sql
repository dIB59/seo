...
SELECT id, url, status, progress, total_pages, analyzed_pages, 
       ...
       ssl_certificate, created_at, created_at as updated_at
...
SELECT id, url, settings_id, created_at, created_at, status, result_id
FROM analysis_jobs;
...
-- Recreate views that were dropped at the start
