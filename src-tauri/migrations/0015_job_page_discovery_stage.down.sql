-- Rollback 'discovering' status

-- Revert analysis_results status to existing ones
-- Note: jobs currently in 'discovering' will need to be handled if we rollback.
-- For simplicity, this script just reverts the schema.
...
-- When rolling back, map any 'discovering' status back to 'analyzing'
INSERT INTO analysis_results_new 
SELECT id, url, 
       CASE WHEN status = 'discovering' THEN 'analyzing' ELSE status END,
       progress, total_pages, analyzed_pages, 
...
-- Map 'discovering' back to 'processing' (or 'queued' depending on preference)
INSERT INTO analysis_jobs_new (id, url, settings_id, created_at, updated_at, status, result_id)
SELECT id, url, settings_id, created_at, updated_at, 
       CASE WHEN status = 'discovering' THEN 'processing' ELSE status END,
       result_id
FROM analysis_jobs;
